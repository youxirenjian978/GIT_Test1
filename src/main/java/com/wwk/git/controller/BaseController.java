...
public class BaseController {
	...
	public ModelAndView getPageNumberInfo(int total,int startIndex,int pageSize ,ModelAndView result) {
        ...
        int current =  (int) Math.ceil((startIndex + 1.0) / pageSize);//4向上取整 代表当前页
		result.addObject("start", startIndex);//9//起始下标
		result.addObject("limit", pageSize);//页面数据大小
		result.addObject("total", total);//总条数
		result.addObject("current", current);//代表当前页
